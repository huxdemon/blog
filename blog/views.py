...
class IndexView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        is_paginated = context.get('is paginated')

        # 调用自己写的pagination_date 方法获得显示分页导航条需要的数据
        pagination_data = self.pagination_data(paginator, page, is_paginated)

        # 将分页导航条的模板变量更新到context 中，注意pagination_data 方法返回的也是一个字典
        context.update(pagination_data)

        # 将更新后的 context 返回，以便 ListView 使用这个字典中的模板变量去渲染模板，
        # 注意此时的 context 字典中已有了显示分页导航条所需的数据。
        return context
...
class PostDetailView(DetailView):
    ...
    # 根据文章的 id（也就是 pk）获取文章，然后对文章的 post.body 进行 Markdown 渲染
    def get_object(self, queryset=None):
        # 覆写get_object 方法的目的是因为需要对post 的body 值进行渲染
